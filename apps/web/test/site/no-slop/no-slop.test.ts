...
// like a website generated by AI. site/AGENTS.md states that as eleven
...
// The marketing half of apps/web and nothing else: the product has its own
// vocabulary and its own tests, and these rules are not written for it.
const SCANNED_DIRS = [join('app', '(site)'), 'site'];
...
test('invariant 10: the domain is written in exactly one place', () => {
  const hits: string[] = [];
  for (const { rel, src } of FILES) {
    if (rel === join('site', 'lib', 'links.ts')) continue;
