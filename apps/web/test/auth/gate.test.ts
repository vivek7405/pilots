...
const PAGES = ['/dashboard/sandboxes', '/dashboard/storage', '/dashboard/domains', '/dashboard/usage', '/dashboard/keys', '/dashboard/org'] as const;

/** The three that moved. Each answers 308 to its new address, signed in or not. */
const MOVED: [string, string][] = [
  ['/dashboard/machines', '/dashboard/sandboxes'],
  ['/dashboard/services', '/dashboard'],
  ['/dashboard/volumes', '/dashboard/storage'],
];
...
test('the home page is the overview once signed in, and the sign-in offer before', async () => {
  const anon = await app.handle(new Request('http://localhost/dashboard'));
  ...
  const signedIn = await app.handle(new Request('http://localhost/dashboard', asUser(cookie)));
...
test('the keys page never renders a stored key value', async () => {
  ...
  const page = await app.handle(new Request('http://localhost/dashboard/keys', asUser(cookie)));
...
// A URL segment is an address. `/dashboard/machines` was in the nav, in the command
// palette and in anyone's bookmarks, so it moves with a permanent redirect
// rather than a 404, while `/dashboard/machines/<id>` keeps its path entirely.
