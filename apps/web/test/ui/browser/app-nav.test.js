...
suite('app-nav', () => {
  ...
  test('the current attribute seeds the first paint', async () => {
    history.replaceState({}, '', '/dashboard/sandboxes');
    const el = await mount('/dashboard/sandboxes');
    assert.equal(lit(el), '/dashboard/sandboxes');
  });

  test('a soft navigation moves the highlight', async () => {
    history.replaceState({}, '', '/dashboard');
    const el = await mount('/dashboard');
    await softNavigate(el, '/dashboard/sandboxes');
    assert.equal(lit(el), '/dashboard/sandboxes', 'the highlight followed the router, not the server render');
  });
  ...
  test('a section owns its subroutes, and Apps does not own everything', async () => {
    ...
    await softNavigate(el, '/dashboard/apps/gallery');
    assert.equal(lit(el), '/dashboard', 'an app page keeps Apps lit');

    // A service's page belongs to Apps, which is where the service was found.
    await softNavigate(el, '/dashboard/services/svc-1');
    assert.equal(lit(el), '/dashboard');
    ...
    await softNavigate(el, '/dashboard/machines/m-1');
    assert.equal(lit(el), '/dashboard/sandboxes');
    await softNavigate(el, '/dashboard/machines/m-1/terminal');
    assert.equal(lit(el), '/dashboard/sandboxes');

    // '/dashboard' is matched exactly, or it would be lit on every page under it.
    await softNavigate(el, '/dashboard/usage');
