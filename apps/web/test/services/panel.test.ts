/**
 * The service panel at its own address, `/dashboard/services/<id>?tab=`.
 *
...
test('the default tab is Deployments, showing what is live and what can be rolled back to', async () => {
  const { status, body } = await page('/dashboard/services/svc-web');
  assert.equal(status, 200);
  assert.match(body, /aria-label="Service sections"/);
  assert.match(body, /href="\/dashboard\/services\/svc-web\?tab=deployments"[^>]*aria-current="page"/);
  ...
  assert.match(deployForm, /name="back" value="\/dashboard\/services\/svc-web\?tab=deployments"/, 'the deploy form carries its return');
...
test('?tab=settings renders the Instances form and only that tab', async () => {
  const { body } = await page('/dashboard/services/svc-web?tab=settings');
  assert.match(body, /href="\/dashboard\/services\/svc-web\?tab=settings"[^>]*aria-current="page"/);
...
test('the Address form is offered only to a service with no address', async () => {
  const withAddress = await page('/dashboard/services/svc-web?tab=settings');
  assert.doesNotMatch(withAddress.body, /name="domain"[^>]*value="web"/);

  const without = await page('/dashboard/services/svc-quiet?tab=settings');
...
test('setting an address patches the service with the label alone', async () => {
  ...
  const res = await submitForm(
    app.handle,
    '/dashboard/services/svc-quiet?tab=settings',
    { service: 'svc-quiet', domain: 'quiet', back: '/dashboard/services/svc-quiet?tab=settings' },
...
test('an address that is not a DNS label never reaches the fleet', async () => {
  ...
  const res = await submitForm(
    app.handle,
    '/dashboard/services/svc-quiet?tab=settings',
    { service: 'svc-quiet', domain: 'Not A Label', back: '/dashboard/services/svc-quiet?tab=settings' },
    { cookies: cookie, match: 'Set address' },
  );
...
test('a form returns to the tab it was on', async () => {
  ...
  const res = await submitForm(
    app.handle,
    '/dashboard/services/svc-web?tab=settings',
    { service: 'svc-web', replicas: '2', back: '/dashboard/services/svc-web?tab=settings' },
    { cookies: cookie, match: 'Save' },
  );
  assert.equal(res.status, 303);
  assert.equal(res.headers.get('location'), '/dashboard/services/svc-web?tab=settings&ok=saved');
...
test('a back that leaves the site is refused and the service page is used instead', async () => {
  ...
  assert.equal(res.status, 303);
  assert.equal(res.headers.get('location'), '/dashboard/services/svc-web?ok=saved');
});
...
test('the keep-awake box becomes the one knob the deploy carries, and nothing when unticked', async () => {
  ...
  let res = await submitForm(
    app.handle,
    '/dashboard/services/svc-web?tab=deployments',
...
test('the Terminal tab is a shell on a running instance, with a full-screen link', async () => {
  const { body } = await page('/dashboard/services/svc-web?tab=terminal');
...
test('a service with nothing running gets an empty Terminal tab pointing at Deployments', async () => {
  ...
  try {
    ...
    assert.match(body, /href="\/dashboard\/services\/svc-web\?tab=deployments"/);
  } finally {
...
test('an unknown tab falls back to Deployments rather than an empty panel', async () => {
  const { body } = await page('/dashboard/services/svc-web?tab=nonsense');
  assert.match(body, /data-current-deployment/);
});
...
test('a machine left on an older release is not counted, but is still named', async () => {
  ...
  try {
    ...
    const { body } = await page('/dashboard/services/svc-web');
    ...
    // that payload whatever the markup shows. `href="/dashboard/machines/<id>"` is only
    // written by a list that actually drew the row.
    const deployments = await page('/dashboard/services/svc-web?tab=deployments');
    assert.match(deployments.body, /href="\/dashboard\/machines\/m-web"/, 'the current release names the instance running it');
    assert.ok(
      !/href="\/dashboard\/machines\/m-web-old"/.test(deployments.body),
    ...
    const metrics = await page('/dashboard/services/svc-web?tab=metrics');
    assert.match(metrics.body, /href="\/dashboard\/machines\/m-web-old"/, 'the leftover is still listed among the instances');
