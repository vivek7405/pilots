...
// runEgress keeps the root namespace's per-org source rewrites matching the
// machines this host is running, and publishes this host's prefix so that any
// host can answer what address a tenant leaves from.
//
// Does nothing at all on a host with no egress configuration, which is every
// host until an operator gives one a prefix. That is not a guard bolted on: a
// wrong rule here is invisible until a tenant's application cannot reach the
// internet, so the feature stays off until it is asked for.
func runEgress(ctx context.Context, hostID string, cfg netns.EgressConfig,
	...
	for {
		...
		} else if fingerprint := plan.Fingerprint(cfg); fingerprint != applied {
			if err := netns.ApplyEgress(cfg, plan); err != nil {
