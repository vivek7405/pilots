...
// EgressConfig is what an operator has to say to turn this on.
//
// Both fields empty is the default and means the feature is OFF: no table is
// created, no address is assigned, and a host behaves exactly as it did before
// egress addresses existed. That is deliberate. Guest outbound traffic is the
// one path where a wrong rule is invisible until a tenant's application stops
// being able to reach the internet, so nothing happens until a host is told to
// make it happen.
...
func (p EgressPlan) Fingerprint(c EgressConfig) string {
	h := sha256.New()
	fmt.Fprintf(h, "C|%s|%s\n", c.Interface, c.Prefix6)
...
//
// A disabled config REMOVES the table rather than leaving whatever generation
// was there. Turning the feature off and having a host keep rewriting source
// addresses would be the worst of both: an operator who thinks they have
// reverted, and traffic that still leaves from addresses nothing documents.
func ApplyEgress(c EgressConfig, plan EgressPlan) error {
	...
	if !c.Enabled() {
		if err := conn.Flush(); err != nil {
			return fmt.Errorf("netns: remove the egress table: %w", err)
		}
		return nil
	}
	...
	for _, r := range plan.Rules {
		...
		conn.AddRule(&nftables.Rule{
			...
			Exprs: concat(
				matchIface(expr.MetaKeyOIFNAME, c.Interface),
