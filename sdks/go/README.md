...
Every `Machine` carries `LastStart` and `LastStartAt`, which say how it last
came up:

| `LastStart` | What happened |
| --- | --- |
...
`Health` also carries `CPUVendor`, which pool that host restores memory images
